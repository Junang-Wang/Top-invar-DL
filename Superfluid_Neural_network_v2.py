# This file contents neural network codes for detecting winding number from superfluid stiffness matrices. 
import torch
import torch.nn as nn
import torch.nn.functional as F
# set up dataset class
class Superfluid_Dataset(torch.utils.data.Dataset):
    def __init__(self,train_x,train_y,train):
        #data loading
        self.x = train_x 
        self.y = train_y 
...
class NN_net(nn.Module):
    ...
    '''
    def __init__(self,Conv_args,fc_args,Conv_block,fc_block,num_classes):
        super().__init__()
        NNstages = []
        ...
        self.total_net = nn.Sequential(
            *NNstages,
            nn.Linear(fc_args[-1][1],num_classes,bias=True)
            )
    def forward(self,x):
...
class Two_Branches_NN_net(nn.Module):
    ...
    def __init__(self,Conv_args,fc_args,Conv_block,fc_block,num_classes):
        ...
        self.stem_1 = nn.Linear(fc_args[-1][1],num_classes,bias=True)
        self.stem_2 = nn.Linear(fc_args[-1][1],num_classes,bias=True)
    def forward(self,x):
      h1 = self.Conv_net(x)
      h2 = self.fc_net_1(h1) 
      h3 = self.fc_net_2(h1)
